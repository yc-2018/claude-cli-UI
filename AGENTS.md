...
- Treat `%USERPROFILE%\.claude\projects` session JSONL files as Claude-owned data. Imported histories must retain their original session IDs. Permitted mutations are normalizing top-level `entrypoint` and `promptSource` fields written by claude-cli-UI after the Claude process exits, and moving a user-confirmed deleted session to the operating system recycle bin. Never alter message content, UUIDs, or tool records.
