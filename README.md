...
项目行右侧的刷新按钮会重新扫描 Claude CLI 会话，并载入 CLI 在应用外追加的消息，不需要关闭并重开应用。会话名称会与 Claude CLI 的自定义标题双向同步，列表同时显示最后更新时间和 Git 分支。将鼠标移到已完成的 Claude 回复上，可从该轮回答结束处创建独立会话分支；新分支会写入 Claude CLI 历史并使用递增编号命名。侧边栏右缘可拖动调整宽度。历史来自当前电脑的 Claude 配置目录（默认 `%USERPROFILE%\.claude\projects`，也支持 `CLAUDE_CONFIG_DIR`）；Portable 程序本身不携带、上传或跨电脑同步这些历史文件。
